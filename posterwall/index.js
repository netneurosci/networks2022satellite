...
const goToRoom = (roomName) => {
  location.href = `https://meet.jit.si/NetNeurosci2022_${roomName}`;
}
...
const displayPosters = (posters) => {
  ...
  for (const p of posters) {
    ...
    // svg += `<g id="nn22-${id}" transform="translate(${p.x},${p.y-13}) scale(0.2)">
    svg += `<g id="nn22-${id}" transform="translate(${p.x - 14*(id==="46"||id==="48")},${p.y - 13 + 12*(id==="46"||id==="48")}) scale(0.2)">
      ...
      fill="rgba(255,255,255,0.4)" onclick="openJit('nn22-${id}', '${name}');" style="cursor:pointer; pointer-events: visiblePainted"></path>
      ...
      fill="white" stroke-width="5" onclick="openJit('nn22-${id}', '${name}');" style="cursor:pointer; pointer-events: visiblePainted"></path>
...
const main = async () => {
  viewer = OpenSeadragon({
    ...
    tileSources: "https://microdraw.pasteur.fr/netneurosci2022/dzi.js"
  });
